...
from PySide2.QtWidgets import QGraphicsItem, QGraphicsLineItem, QGraphicsEllipseItem, QVBoxLayout, QGraphicsItemGroup
    ...
    def update_skeleton(self, skeleton:Skeleton):
        for i,part in enumerate(self.config['body_parts']):
            if skeleton[part]>=self.threshold:
                ...
                self.markers[part].setToolTip(f'{part:10} ({round(skeleton[part].likelihood,2)})\n{int(skeleton[part][0]):5},{int(skeleton[part][1]):5}')
            else:
