...
class OptiPoseWidget(QWidget):
    def __init__(self,config,session_manager:SessionFileManager):
        ...
        self.ui.recon_scale.setText(str(self.config['OptiPose'].get('scale',1.0)))
        self.ui.recon_threshold.setValue(self.config['OptiPose'].get('threshold',0.9))
        self.ui.median_threshold.setValue(self.config.get('threshold',0.6))
        self.ui.interp_threshold.setValue(self.config.get('threshold', 0.6))
        self.ui.kalman_threshold.setValue(self.config.get('threshold', 0.6))
        self.ui.moving_avg_threshold.setValue(self.config.get('threshold', 0.6))
    ...
    def start_pipeline(self,event=None):
        ...
        if os.path.exists(path):
            btn = QMessageBox.information(self.ui,"File Exists!","File Exists; Do you want to overwrite?",QMessageBox.Yes,QMessageBox.No)
    ...
    def update_alignment_data(self,event=None):
        ret=OptiPose.pipeline.update_alignment_matrices(self.config)
        if ret:
    ...
    def insert_input_file(self,event=None):
        ...
        if self.ui.custom_file_btn.isChecked():
            ...
            for candidate in candidate_views:
                ...
                camera_config[candidate]=self.config['views'][candidate]
            post_processor = ReconstructionProcess(self.config,camera_config,data_readers,self.ui.recon_threshold.value(),scale=float(self.ui.recon_scale.text()),
                                            reconstruction_algorithm=self.ui.recon_algorithm.currentText(),file_prefix=self.ui.recon_file_prefix.text())
        self.generate_process_card([post_processor])
    ...
    def insert_kalman_filter_process(self,event):
        ...
        post_processors = []
        for column in self.config['body_parts']:
            post_processors.append(KalmanFilterProcess(column,self.config['OptiPose']['framerate'],skip_invalid,threshold))
        self.generate_process_card(post_processors)
